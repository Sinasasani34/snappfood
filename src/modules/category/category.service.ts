...
import { toBoolean, isBoolean } from 'src/common/utility/functions.utils';
...
export class CategoryService {
  ...
  async create(createCategoryDto: CreateCategoryDto, image: Express.Multer.File) {
    const { Location, Key } = await this.s3Service.uploadFile(image, 'snapfood-image');
    let { title, slug, parentId, show } = createCategoryDto;
    const category = await this.findOneBySlug(slug)
    if (category) throw new ConflictException('اين دسته بندي از قبل وجود دارد!!!');
    if (isBoolean(show)) {
      show = toBoolean(show);
    }

    let parent: CategoryEntity | any = null;
    ...
    return {
      message: 'دسته بندي با موفقيت ايجاد شد'
    }
  }
  ...
  async findOneById(id: number) {
    ...
    if (!category) throw new NotFoundException('دسته بندي يافت نشد')
    return category;
  }
  ...
  async remove(id: number) {
    ...
    return {
      message: "دسته بندي با موفقيت حذف گرديد"
    }
  }
  ...
  async findBySlug(slug: string) {
    ...
    if (!category) throw new NotFoundException('دسته بندي يافته نشد بر اساس اسلاگ');
    return {
      category
    }
  }
